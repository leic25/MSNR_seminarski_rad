...
\setlength{\labelsep}{10pt}
	\item Optimizacija rojevima čestica {\em(eng. Particle Swarm Optimization, PSO)},
...
Nabrojani algoritmi rade nad skupom jedinki - rojem. Elementi ovog skupa nazivaju se čestice. Čestice se na unapred definisan način kreću po prostoru pretraživanja. Njihovo kretanje se usmerava imajući u vidu njihovu trenutnu poziciju, njihovu trenutno najbolju poziciju, kao  najbolju poziciju čitavog roja. Na ovaj način stvara se model koji je prilagođen računaru, kao i potrebama određenih zadataka. 
...
\section{Zaključak}
\label{sec:zakljucak}
Optimizacija kolonijom pčela je metaheuristička metoda inspirisana ponašanjem pčela, kao i najmlađa tehnika koja koristi inteligenciju roja. Predstavljen je opšti algoritam koji se, uz određene mofikacije, može prilagoditi za različite probleme optimizacije u upravljanju, inženjeringu i/ili kontroli. BCO ima mogućnost da, putem razmene informacija i regrutovanja, intenzivnije vrši pretragu u onim regionima prostora rešenja koji više obećavaju. Ipak, BCO nije široko korišćen za rešavanje stvarnih problema.

{\em U ovom trenutku nema teorijskih rezultata koji bi mogli podržati koncepte BCO. Obično je razvoj različitih pristupa inteligenciji roja zasnovan na eksperimentalnom radu u početnoj fazi. Dobri eksperimentalni rezultati trebalo bi da motivišu istraživače da pokušaju da proizvedu neke teorijske rezultate u budućem istraživanju. 
